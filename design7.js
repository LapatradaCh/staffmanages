...
function renderDesign7(filtered, pendingFiltered, pendingList, staffList) {
    ...
    if(d7Tab === 'pending') {
            ...
            detailEl.innerHTML = `
                <div class="flex flex-col h-full">
                    ...
                    <div class="flex gap-2 pt-6">
                        <button onclick="showPendingInfo('${p.name}', '${p.phone}')" class="flex-1 bg-slate-50 border border-slate-200 hover:bg-slate-100 text-slate-700 font-bold py-3.5 rounded-xl text-[11px] md:text-xs flex items-center justify-center transition-colors">
                            <span class="material-icons text-[14px] mr-1.5">info</span> ข้อมูล
                        </button>
                        <button onclick="handleReject(${p.id})" class="flex-[1] bg-rose-50 border border-rose-100 hover:bg-rose-100 text-rose-600 font-bold py-3.5 rounded-xl text-[11px] md:text-xs flex items-center justify-center transition-colors">ปฏิเสธ</button>
        ...
        if(s) {
            ...
            // แยกตัวแปร Responsibilities (ป้องกัน Code แดงจาก Backtick ซ้อนกัน)
            let respHtml = '';
            ...
            // แยกตัวแปร สถานะ Authen
            let isAuthen = s.isAuthen !== false;
            let authenCardHtml = '';

            // ถ้าผ่านการ Authen จะเป็นสีเขียว ถ้ายังไม่ผ่าน จะเป็นสีเทา (กล่องอื่นไม่ต้องเปลี่ยน)
            if (isAuthen) {
                authenCardHtml = '<div class="p-3 bg-emerald-50/50 border border-emerald-100/50 rounded-xl flex flex-col gap-1.5">' +
                ...
                authenCardHtml = '<div class="p-3 bg-slate-50 border border-slate-200/60 rounded-xl flex flex-col gap-1.5">' +
            ...
            // นำกล่อง Authen มาประกบรวมกับกล่อง น้ำเงิน (เข้าครั้งแรก) และเหลือง (ใช้งานล่าสุด) ที่คงสีไว้เสมอ
            let statusCardsHtml = authenCardHtml + 
            ...
            let btnRoleToggle = s.role === 'admin' ? 'ลดสถานะ' : 'เพิ่มเป็นผู้ดูแล';
                    ...
                    <div class="flex items-start gap-5 pb-6">
                        ...
                        <div class="min-w-0 flex-1 pt-1">
                            ...
                            <div class="flex items-center gap-2">
                                <span class="inline-block ${roleBadgeClass} px-3 py-1 rounded-full text-[10px] font-bold">${roleNameDetail}</span>
